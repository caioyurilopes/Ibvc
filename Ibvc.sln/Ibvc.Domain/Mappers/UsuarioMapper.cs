    ...
    public static Usuario ToEntity(this CadastrarMembroRequest request)
    {
        return new Usuario
        {
            ...
            Batismo = request.DataBatismo.HasValue || !string.IsNullOrEmpty(request.IgrejaBatismo)
                ...
                : null,
            ProfissaoDeFe = request.DataProfissaoFe.HasValue || !string.IsNullOrEmpty(request.IgrejaProfissaoFe)
                ? new ProfissaoDeFe
                {
                    Data = request.DataProfissaoFe ?? default,
                    Igreja = request.IgrejaProfissaoFe ?? string.Empty,
                    PastorId = request.PastorProfissaoFeId,
                    PastorNomeManual = request.PastorProfissaoFeNomeManual
                }
                : null
        };
    }
